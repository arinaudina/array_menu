#include <stdio.h>
#include <time.h>
#include <math.h>

// Подфункции для ввода данных в массив

int manual_input(double arr[], int size, int *error)            //Ручной ввод
{
    printf("\n  Введите элементы массива:\n");

    if (error == NULL)
        return -1;

    for (int i = 0; i < size; i++) 
    {
        if (scanf("%lf", &arr[i]) != 1)
        {
            *error = -1;
            return -1;
        }
    }

    return size;
}


int random_full_input(double arr[], int size)       //Рандомные числа в полном диапазоне
{
    srand(time(NULL)); //сбрасывает random при входе в функцию

    for (int i = 0; i < size; i++) 
    {
        arr[i] = ((double)rand() / RAND_MAX * 20000 - 10000);
    }

    return size;
}


int random_natural_input(double arr[], int size)    // Рандомные числа больше 0
{
    srand(time(NULL));

    for (int i = 0; i < size; i++)
	{
		arr[i] = ((double)rand() / RAND_MAX * 10000);
	}

    return size;
}


int random_range_input(double arr[], int size, int *error)      // Рандомные числа в диапазоне [min, max]
{              
    int min = 0;
    int max = 0;

    if (error == NULL)
        return -1;

    srand(time(NULL));

	do
    {
        printf("\n  Введите целое минимальное значение: ");
	    if(scanf("%d", &min) !=1)
        {
            *error = -1;
            return -1;
        }
	    printf("  Введите целое максимальное значение: ");
	    if(scanf("%d", &max) != 1)
        {
            *error = -1;
            return -1;
        }
    } while (min >= max); //Получение корректных значений от пользователя

	for (int i = 0; i < size; i++)				// Заполнение массива случайными числами в диапазоне [min, max]
	{
		arr[i] = (double)rand() / RAND_MAX * (max - min) + min;
	}

    return size;
}


int arithmetic_progression_input(double arr[], int size, int *error)      // Арифметическая прогрессия
{
    double start = 0.;
    double step = 0.;

    if (error == NULL)
        return -1;

		printf("\n  Введите начальное число: ");
	    if(scanf("%lf", &start) !=1)
        {
            *error = -1;
            return -1;
        }
	    do
        {
            printf("  Введите шаг прогрессии: ");
	        if(scanf("%lf", &step) != 1)
            {
                *error = -1;
                return -1;
            }
        } while(step == 0);      //чтоб не заполнять массив одинаковыми числами

		for (int i = 0; i < size; i++)				
		{
			arr[i] = start + i * step;				//aₙ = a₁ + (n-1) * d
		}

    return size;
}


int geometric_progression_input(double arr[], int size, int *error)     // Геометрическая прогрессия
{
    double start = 0.;
    double ratio = 0.;

    if (error == NULL)
        return -1;

        do
        {
		    printf("\n  Введите начальное число: ");
            if(scanf("%lf", &start) !=1)
            {
                *error = -1;
                return -1;
            }
        } while(start == 0);
        do
        {
		    printf("  Введите знаменатель прогрессии: ");
            if(scanf("%lf", &ratio) != 1) //Проверка
            {
                *error = -1;
                return -1;
            }
        } while(ratio == 0);         //Чтоб не заполнять массив одинаковыми числами

		arr[0] = start;							
		for (int i = 1; i < size; i++)
        {
			arr[i] = arr[i - 1] * ratio;		// aₙ = a₁ * qⁿ⁻¹
		}

        return size;
}


int squares_num_input(double arr[], int size, int *error)     // Квадраты чисел
{
    double start = 0.;
    int answer = 0;

    if (error == NULL)
        return -1;
			
    printf("\n  Введите начальное число: ");
	if(scanf("%lf", &start) != 1)
    {
        *error = -1;
        return -1;
    }

    printf ("\n- Введите 1 если хотите заполнять массив по возрастанию\n- Введите 2 если хотите заполнять массив по убыванию\n");
    do
    {
        printf("\n  Ваш ответ: ");

        if(scanf("%d", &answer) != 1)
        {
            *error = -1;
            return -1;
        }
        
        switch (answer)
        {
            case 1:
            {
	            for (int i = 0; i < size; i++)			
	            {
		            arr[i] = (start + i) * (start + i);
	            }

                break;
            }
            case 2:
            {
                for (int i = 0; i < size; i++)			
	            {
		            arr[i] = (start - i) * (start - i);
	            }

                break;
            }
        }
    } while ((answer != 1) && (answer != 2));

    return size;
}


int pow_num_input(double arr[], int size, int *error)       // Массив степени числа
{
    double start = 0.;
    double start_power = 0.;
    int answer = 0;

    if (error == NULL)
        return -1;

	do
    {
        printf("\n  Введите число: ");
        if(scanf("%lf", &start) != 1)
        {
            *error = -1;
            return -1;
        }
    } while((start == 0) || (start == 1));
	printf("  Введите начальную степень: ");
    if(scanf("%lf", &start_power) != 1)
    {
        *error = -1;
        return -1;
    }

    printf ("\n- Введите 1 если хотите заполнять массив по возрастанию степени\n- Введите 2 если хотите заполнять массив по убыванию степени\n");
    do
    {
        printf("\n  Ваш ответ: ");

        if(scanf("%d", &answer) != 1)
        {
            *error = -1;
            return -1;
        }
        
        switch (answer)
        {
            case 1:
            {
	           for (int i = 0; i < size; i++)	
				{
					arr[i] = pow(start, start_power + i);
				}

                break;
            }
            case 2:
            {
                for (int i = 0; i < size; i++)	
				{
					arr[i] = pow(start, start_power - i);
				}

                break;
            }
            default:
            {
                printf("\nНекоректный вариант. Попробуйте снова\n");
                break;
            }
        }
    } while ((answer != 1) && (answer != 2));

    return size;
}


// Функционал ввода данных в массив:


int input(double arr[], int size, int manual_limit, int *error) 
{   
    if (error == NULL)
        return -1;
    
    printf("\nИнструкция по заполнению массива:\n");
	printf(" - Для заполнения массива необходимо указать его размер\n");
    printf(" - При указании размера массива введите любое число от 1 до 200\n");
    printf("    Если размер массива 20 и меньше - вам будет доступен ручной ввод\n");
    printf("    Если размер массива больше 20 - вам будет доступно меню с вариантами заполнения массива\n");
    printf(" - Введите 1 если хотите продолжить или 0 если хотите вернуться назад в главное меню\n \n");

    int forward_back = 0;
    do                                              //Ожидание ответа от пользователя
    { 
        printf("Введите требуемое значение: ");
        if (scanf("%d",&forward_back) != 1)
        {
            *error = -1;
			return -1;
        }
    } while ((forward_back != 0) && (forward_back != 1));
    if (forward_back == 0)
    {
        return 0;
    }
    
    int user_size = 0;
    do												//Количество элементов в массиве
	{
		printf("\nВведите размер массива: ");
		if(scanf("%d", &user_size) != 1)
		{
            *error = -1;
			return -1;
		}
	} while ((user_size < 1) || (user_size > size));


	if (user_size <= manual_limit)									//Вариативность заполнения
	{
        *error = 0;
        user_size = manual_input(arr, user_size, error);
	}
	else
	{
        int answer = 0;
        do
        {
            *error = 0;

		    printf(
		        "\nВарианты заполнения массива:\n"
    	        " 1. Массив со случайными числами в полном диапазоне\n"
  		        " 2. Массив со случайными неотрицательными числами\n"
  		        " 3. Массив чисел от _ до _\n"
 		        " 4. Массив арифметической прогрессии\n"
  		        " 5. Массив геометрической прогрессии\n"
  		        " 6. Массив квадратов чисел\n"
  		        " 7. Массив степени числа\n\n"
 		        "Выберите как заполнить массив: "
                );

            if (scanf("%d", &answer) != 1)
            {
                *error = -1;
                return -1;
            }

		    switch (answer)								// Обработка запроса пользователя
		    {
			    case 1:
                    user_size = random_full_input(arr, user_size);
				    break;

			    case 2:
				    user_size = random_natural_input(arr, user_size);
				    break;

			    case 3:
                    user_size = random_range_input(arr, user_size, error);
				    break;

			    case 4:
                    user_size = arithmetic_progression_input(arr, user_size, error);
				    break;

			    case 5:
				    user_size = geometric_progression_input(arr, user_size, error);
				    break;

			    case 6:
				    user_size = squares_num_input(arr, user_size, error);
				    break;

			    case 7:
				    user_size = pow_num_input(arr, user_size, error);
				    break;

                default:
                    printf("\nНекоректный вариант. Попробуйте снова\n");
                    break;
            }

        } while ((answer < 1) || (answer > 7));

	} 

    return user_size;
}
   